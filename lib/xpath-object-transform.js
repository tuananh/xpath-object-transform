(function(root, factory) {
    ...
    function walk(data, path, result, key, parentpath, parentObj) {
        ...
        if (path.toString().startsWith('^')) {
            handleGoingUp(data, path, result, key, parentpath, parentObj)
        } else {
            switch (type(path)) {
                case 'string':
                    fn = seekSingle
    ...
    function handleGoingUp(data, path, result, key, parentpath, parentObj) {
        ...
        // fn = seekSingle
        seekSingle(parentObj, goUpPath, result, key)
    }
    ...
    function seekArray(doc, pathArr, result, key, parentpath, parentObj) {
        // NOTE: when array loop with current node, set path to parentpath
        let path = pathArr[0],
            subpath = pathArr[1],
            seek = doc.find(path)
